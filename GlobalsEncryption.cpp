...
void GlobalsEncryption::process(Function &F)
{
    ...
    Function *DecFunc = buildDecryptFunction(*F.getParent());
    for (GlobalVariable *GV : GVs)
    {
        ...
        uint32_t K = RandomEngine.get_uint32_t();
        ...
        for (Function *F : Funcs)
        {
            ...
            IRB.CreateCall(FunctionCallee(DecFunc),
                           {
                               IRB.CreateBitOrPointerCast(Copy, IRB.getInt8Ty()->getPointerTo()),
                               IRB.CreateBitOrPointerCast(Key, IRB.getInt8Ty()->getPointerTo()),
                               IRB.getInt64(Size),
